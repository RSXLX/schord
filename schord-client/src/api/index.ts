import { getBaseURL, get, post, deletes,posta } from "./request";
...
const HttpManager = {
  ...
  // 更新用户头像 
  uploadUrl: (userId) => post(`user/avatar/update?id=${userId}`),
  ...
  getBannerList: () => get("banner/getAllBanner"),
      ...
      // =======================> 讨论区 API
    // 返回所有讨论区信息
    getAllDiscussion: () => get(`discussion`),
    // 删除讨论区信息
    deleteDiscussion: (id) => get(`discussion/${id}`),
    //添加一条信息
  addDiscussion: (discussion:any) => post(`discussion/add`, discussion),  };
