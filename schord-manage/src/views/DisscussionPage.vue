<template>
  ...
  <div class="container">
    <div class="handle-box">
      ...
      <el-input v-model="searchWord" placeholder="根据用户搜索说过的内容"></el-input>
    </div>
    <el-table height="550px" border size="small" :data="tableData" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="content" label="讨论内容"></el-table-column>
...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = ref([]); // 记录讨论区，用于显示
    const tempDate = ref([]); // 记录讨论区，用于搜索时能临时记录一份讨论区列表
    ...
    function getData() {
      tableData.value = [];
      tempDate.value = [];
      HttpManager.getAllDiscussion().then((res) => {
  ...
  const result = (await HttpManager.deleteDiscussion(idx.value)) as ResponseBody;
